...
<body>
	...
	<div class="cart-wrapper">
		...
		%>
		<div class="cart-top">
			<button onclick="window.location.href='homepage.jsp'">CONTINUE
				SHOPPIING</button>
			<%
			if (total > 0) {
			%>
			<button onclick="window.location.href='paymentaddress.jsp'">CHECKOUT
				NOW</button>
			<%
			}
			%>
		...
		<div class="cart-bottom">
			<div class="cart-info">
				...
				pstmt2 = conn.prepareStatement(
						"select * from product inner join cart on product.pid = cart.product_id where cart.email=? and cart.status is null");
					...
					<div class="product-details">
						...
						<div class="total-product">
							...
							<div class="product-amount">
								<i class="fa-solid fa-minus quantity-symbol"
									onclick="incdec('dec',<%=rs2.getInt("product_id")%>,<%=rs2.getInt("quantity")%>)"></i>
								&nbsp;<%=rs2.getString("quantity")%>&nbsp; <i
									class="fa-sharp fa-solid fa-plus quantity-symbol"
									onclick="incdec('inc',<%=rs2.getInt("product_id")%>,<%=rs2.getInt("quantity")%>)"></i>
							</div>
						</div>
					</div>
					<div class="product-price">
						...
						<div class="remove-btn">
							<form action="removeCartAction" method="post">
								<input type="hidden" value="<%=rs2.getInt("product_id")%>"
									name="productId">
